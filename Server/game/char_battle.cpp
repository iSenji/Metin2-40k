...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type) // returns true if dead
{
	...
	if (DAMAGE_TYPE_MAGIC == type || DAMAGE_TYPE_NORMAL_MAGIC == type)
#else
	...
	//
#ifdef ADVANCED_BLOCK_DODGE
	else if (type == DAMAGE_TYPE_NORMAL || type == DAMAGE_TYPE_NORMAL_RANGE || type == DAMAGE_TYPE_NORMAL_MAGIC)
#else
...
#ifdef ADVANCED_BLOCK_DODGE
			NormalAttackAffect(pAttacker, this);
#endif
		}
...
#ifdef ADVANCED_BLOCK_DODGE
		else if (type == DAMAGE_TYPE_NORMAL_MAGIC)
		{
			if (GetPoint(POINT_MAGIC) && number(1, 100) <= GetPoint(POINT_MAGIC))
			{
				if (test_server)
				{
					pAttacker->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s 회피! (%d%%)"), GetName(), GetPoint(POINT_MAGIC));
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s 회피! (%d%%)"), GetName(), GetPoint(POINT_MAGIC));
				}
	...
	if (!IsPC())
	{
		StopReturnRecoveryEvent();
	}
#endif
#ifdef ENABLE_MOB_MOVMENT_CONTROLL
	if (!IsPC() && !CheckStoneSpawnEmpty() && IsStone())
	{
		SendDamagePacket(pAttacker, 0, DAMAGE_BLOCK);
		return false;
	}
...
#ifdef ADVANCED_BLOCK_DODGE
						pkVictim->Damage(m_me, iDam, DAMAGE_TYPE_NORMAL_MAGIC);
#else
