...
namespace fishing
{
	...
	void FishingTimer(LPCHARACTER ch, BYTE fishingType, BYTE fishingTime)
	{
		TPacketGCFishingTimer p{};
		p.header = HEADER_GC_FISHING_TIMER;
		p.subHeader = fishingType;
		p.timer = fishingTime;
	...
	#ifdef ENABLE_FISHING_TIMER
				FishingTimer(ch, FISHING_TIMER_SUBHEADER_GC_GET, 6);
	#endif
	...
	#ifdef ENABLE_FISHING_TIMER
		FishingTimer(ch, FISHING_TIMER_SUBHEADER_GC_WAITING, time);
	#endif
	...
	{
	#ifdef ENABLE_FISHING_TIMER
		FishingTimer(ch, FISHING_TIMER_SUBHEADER_GC_RESET, 0);
	#endif
		...
		if (info->step == 1)	// ���Ⱑ �ɸ� ���¸�..
		{
			...
			switch (ret)
			{
				...
				case -3: // ���̵� ������ ����
				case -1: // �ð� Ȯ�� ������ ����
					//ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("���Ⱑ �̳��� ���԰� ��ΰ� ����Ĩ�ϴ�."));
					{
						int map_idx = ch->GetMapIndex();
						int prob_idx = GetProbIndexByMapIndex(map_idx);
