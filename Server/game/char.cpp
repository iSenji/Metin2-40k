	...
	else if (IsPC())
	{
		...
		for (int i = 0; i < WEAR_MAX_NUM; ++i)
			if ((pkItem = GetWear(i)) && pkItem->GetType() == ITEM_ARMOR)
			{
				...
				{
				#ifdef ENABLE_RARITY_SYSTEM
					int rarity = int(pkItem->GetSocket(ITEM_RARITY_SOCKET));
					iArmor += pkItem->GetValue(1)* RARITY_VALUES[rarity];
					iArmor += (pkItem->GetValue(5) * 2) * RARITY_VALUES[rarity];
				#else
					iArmor += pkItem->GetValue(1);
					iArmor += pkItem->GetValue(5) * 2;
				#endif
				}
			}
...
#ifdef ENABLE_MOB_MOVMENT_CONTROLL
EVENTFUNC(return_recovery_event)
{
	char_event_info* info = dynamic_cast<char_event_info*>( event->info );
	if ( info == NULL )
	{
		sys_err( "return_recovery_event> <Factor> Null pointer" );
		return 0;
	}
	...
	if (ch->m_pkReturnRecoveryEvent == NULL)
		return 0;
	...
	if (!ch->IsPC())
	{
		...
		if (ch->GetHP() >= ch->GetMaxHP())
		{
			ch->m_pkReturnRecoveryEvent = NULL;
			return 0;
		}

		ch->PointChange(POINT_HP, MAX(1, (ch->GetMaxHP() * 20) / 100));

		return PASSES_PER_SEC(1);
	}
}

void CHARACTER::StartReturnRecoveryEvent()
{
	if (m_pkReturnRecoveryEvent)
		return;
	...
	m_pkReturnRecoveryEvent = event_create(return_recovery_event, info, PASSES_PER_SEC(1));
}

void CHARACTER::StopReturnRecoveryEvent()
{
	this->m_pkReturnRecoveryEvent = NULL;
}
#endif
...
#ifdef ENABLE_MOB_MOVMENT_CONTROLL
bool CHARACTER::CheckStoneSpawnEmpty()
{
...
#ifdef ENABLE_TARGET_AFFECT
void CHARACTER::BroadcastTargetAffectPacket()
{
	if (m_set_pkChrTargetedBy.empty())
		return;
	...
	while (it != m_set_pkChrTargetedBy.end())
	{
		...
		if (!pkChr->m_list_pkAffect.empty())
			continue;

		itertype(m_list_pkAffect) cit = pkChr->m_list_pkAffect.begin();

		while (cit != pkChr->m_list_pkAffect.end())
		{
			CAffect * pkAff = *cit++;
...
bool CHARACTER::Follow(LPCHARACTER pkChr, float fMinDistance)
{
	...
	if (pkChr->IsPC()) // �Ѿư��� ��밡 PC�� ��
	{
		...
		if (!GetParty() || !GetParty()->GetLeader() || GetParty()->GetLeader() == this)
		{
			...
			if (2500 < DISTANCE_APPROX(m_pkMobInst->m_posLastAttacked.x - GetX(), m_pkMobInst->m_posLastAttacked.y - GetY()))
				if (Return())
					return true;
		...
		if (pkChrProtege)
			if (2500 < DISTANCE_APPROX(GetX() - pkChrProtege->GetX(), GetY() - pkChrProtege->GetY()))
				if (Return())
					return true;
#endif
	}
...
{
	return (GetWear(WEAR_COSTUME_PET));
}
#endif
...
void CHARACTER::CheckMount(bool doRide)
{
	CMountSystem* mountSystem = GetMountSystem();
	...
	if(mountSystem->CountSummoned() == 1 && doRide)
	{
