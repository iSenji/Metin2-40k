...
int CalcMeleeDamage(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, bool bIgnoreDefense, bool bIgnoreTargetRating)
{
	...
	if (pWeapon)
	{
		int rarity = int(pWeapon->GetSocket(ITEM_RARITY_SOCKET));
		iDam += iDam * RARITY_VALUES[rarity];
	}
#endif
...
#ifdef ENABLE_RARITY_SYSTEM
		int rarity = int(pWeapon->GetSocket(ITEM_RARITY_SOCKET));
		iAtk += (pWeapon->GetValue(5) * 2) * RARITY_VALUES[rarity];
#else
...
int CalcArrowDamage(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, LPITEM pkBow, LPITEM pkArrow, bool bIgnoreDefense)
{
...
#ifdef ENABLE_RARITY_SYSTEM
	int rarity = int(pkBow->GetSocket(ITEM_RARITY_SOCKET));
	iDam += iDam * RARITY_VALUES[rarity];
#endif
	int iAtk;
	...
	// Refine Grade
#ifdef ENABLE_RARITY_SYSTEM
	iAtk += (pkBow->GetValue(5) * 2)*RARITY_VALUES[rarity];
#else
