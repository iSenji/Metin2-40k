...
namespace quest 
{
	...
	int pc_mount(lua_State* L)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "pc_mount");
	...
	int pc_unmount(lua_State* L)
	{
		...
		ch->RemoveAffect(AFFECT_MOUNT);
		ch->RemoveAffect(AFFECT_MOUNT_BONUS);
		ch->ChatPacket(CHAT_TYPE_INFO, "pc_unmount");
		if (ch->IsHorseRiding())
			ch->StopRiding();
		return 0;
	}
...
teleport_area:
	...
	int pc_give_rarity_item(lua_State* L) //If it does not compile, edit "ALUA" to "int pc_instant_equip(lua_State * L)".
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		...
		if (lua_isnumber(L,2) && lua_tonumber(L,2)>0)
		{
			iRarity = (int)rint(lua_tonumber(L,2));
			if (iRarity<=0)
			{
				sys_err("QUEST Make item call error : wrong item rarity : %g", lua_tonumber(L,2));
				lua_pushnumber (L, 0);
				return 1;
			}
		}
		...
		LPITEM item = ch->AutoGiveItem(dwVnum, 1);
	...
	void RegisterPCFunctionTable()
	{
		luaL_reg pc_functions[] = 
		{
			...
			{ "get_killee_drop_pct",	pc_get_killee_drop_pct	}, /* mob_vnum.kill 이벤트에서 killee와 pc와의 level 차이, pc의 프리미엄 드랍률 등등을 고려한 아이템 드랍 확률.
																	...
																	*/
#ifdef ENABLE_RARITY_SYSTEM
			{ "give_rarity_item",	pc_give_rarity_item },
#endif
