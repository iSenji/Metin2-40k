...
struct FuncPurge
{
	...
	void operator () (LPENTITY ent)
	{
		...
		if (pkChr->IsNPC() 
			...
			&& !pkChr->IsMount()
#endif
			&& !pkChr->IsPet() && pkChr->GetRider() == NULL)
		{
...
ACMD(do_rarity_item)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Usage: item <item vnum> <rarity-id>");
		return;
	}
	...
	if (*arg2)
	{
		str_to_number(iRarity, arg2);
		iRarity = MINMAX(0, iRarity, 3);
	}
	...
	LPITEM item = ITEM_MANAGER::instance().CreateItem(dwVnum, 1, 0, true);
	...
	if (item)
	{
		switch(item->GetType())
		{
			...
			{
				int iEmptyPos = ch->GetEmptyInventory(item->GetSize());
				item->SetSocket(ITEM_RARITY_SOCKET, iRarity);
				if (iEmptyPos != -1)
				{
					item->AddToCharacter(ch, TItemPos(INVENTORY, iEmptyPos));
					LogManager::instance().ItemLog(ch, item, "GM", item->GetName());
			...
			default:
				ch->ChatPacket(CHAT_TYPE_INFO, "#%u item is not correct type.", dwVnum);
				break;
		}
	}
	else
