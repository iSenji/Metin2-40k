...
void SetPolyVarForAttack(LPCHARACTER ch, CSkillProto * pkSk, LPITEM pkWeapon)
{
	if (ch->IsPC())
	{
		if (pkWeapon && pkWeapon->GetType() == ITEM_WEAPON)
		{
			...
			int rarity = int(pkWeapon->GetSocket(ITEM_RARITY_SOCKET));
			iWep = iWep * RARITY_VALUES[rarity];

			iMtk = iMtk * RARITY_VALUES[rarity];
#endif
	...
	void operator () (LPENTITY ent)
	{
		...
		if (!pkChrVictim->Damage(m_pkChr, iDam, dt) && !pkChrVictim->IsStun())
		{
				...
				!IS_SET(pkChrVictim->GetAIFlag(), AIFLAG_NOMOVE))
			{
...
#ifdef ENABLE_BOSS_NO_FALLBACK
				if (!pkChrVictim->IsNPC() || pkChrVictim->GetMobRank() != MOB_RANK_BOSS)
				{
				...
				{
					m_pkChr->ChatPacket(CHAT_TYPE_INFO, "[ENABLE_BOSS_NO_FALLBACK]: pkChrVictim->GetMobRank(): %d", pkChrVictim->GetMobRank());
				}
#else
					pkChrVictim->Sync(tx, ty);
					pkChrVictim->Goto(tx, ty);
