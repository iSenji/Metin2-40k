...
void CPetActor::SetName()
{
	...
	const char *czMobName = m_pkChar->GetName();
	const char *czPlayerName = m_pkOwner->GetName();

	if (!czMobName || !czPlayerName)
	{
		m_pkChar->SetName("...");
		return;
	}

	std::string mobName(czMobName);
	std::string playerName(czPlayerName);

	std::string petName = playerName + " - " + mobName;

	m_name = petName;
	m_pkChar->SetName(m_name);
}
...
DWORD CPetActor::Summon(LPITEM pSummonItem, bool bSpawnFar)
{
	...
	for (int i = 0; i < ITEM_APPLY_MAX_NUM; ++i)
	{
		...
		m_pkOwner->AddAffect(AFFECT_PET_SUMMON, 0, 0, AFF_NONE, 60 * 60 * 24 * 365, 0, false);
	}
...
{
	bool bResult = true;
	if (NULL == ITEM_MANAGER::instance().FindByVID(this->GetSummonItemVID()) || ITEM_MANAGER::instance().FindByVID(this->GetSummonItemVID())->GetOwner() != this->GetOwner())
	{
		this->Unsummon();
		return true;
	}
