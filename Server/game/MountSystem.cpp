...
bool CMountActor::Mount(LPITEM mountItem)
{
	...
	m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "Mount");
...
void CMountActor::Unmount()
{
	...
	m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "Unmount");
...
DWORD CMountActor::Summon(LPITEM pSummonItem, bool bSpawnFar)
{
	...
	m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "Summon");
...
bool CMountActor::Update(DWORD deltaTime)
{
	...
	if (NULL == ITEM_MANAGER::instance().FindByVID(this->GetSummonItemVID())
