...
LPITEM ITEM_MANAGER::CreateItem(DWORD vnum, DWORD count, DWORD id, bool bTryMagic, int iRarePct, bool bSkipSave)
{
	...
	switch (item->GetType())
	{
		...
		case ITEM_WEAPON:
		{
			...
			if (num >= 100 - RARITY_CHANCE_PCT[2])
				item->SetSocket(ITEM_RARITY_SOCKET, 3);
			else if (num >= 100 - RARITY_CHANCE_PCT[1])
				item->SetSocket(ITEM_RARITY_SOCKET, 2);
			else if (num >= 100 - RARITY_CHANCE_PCT[0])
				item->SetSocket(ITEM_RARITY_SOCKET, 1);
		}
	}
#endif
...
#ifdef ENABLE_RARITY_SYSTEM
		if (pkOldItem->GetSocket(ITEM_RARITY_SOCKET) >= 0)
			pkNewItem->SetSocket(ITEM_RARITY_SOCKET, pkOldItem->GetSocket(ITEM_RARITY_SOCKET));
#endif
