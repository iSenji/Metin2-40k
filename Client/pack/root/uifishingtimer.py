...
class FishingTimerWindow(ui.ScriptWindow):
	def __init__(self):
    ui.ScriptWindow.__init__(self)
		...
		self.fish_time = 0
		self.timeType = player.FISHING_TIMER_SUBHEADER_GC_RESET
	...
	def OnUpdate(self):
		if self.timeType == player.FISHING_TIMER_SUBHEADER_GC_RESET:
			return

		showTime = int(self.fish_time - app.GetGlobalTimeStamp())

		if showTime <= 0 and self.timeType != player.FISHING_TIMER_SUBHEADER_GC_WAITING:
			self.Close()
			return
    ...
    minutes = showTime // 60
		...
		if self.timeType == player.FISHING_TIMER_SUBHEADER_GC_WAITING:
			self.UI["timer"].SetPackedFontColor(ui.GenerateColor(255, 255, 255))
			self.UI["ImageBox"].LoadImage("d:/ymir work/ui/game/fishing_timer/waiting.tga")
		elif self.timeType == player.FISHING_TIMER_SUBHEADER_GC_GET:
			self.UI["timer"].SetPackedFontColor(ui.GenerateColor(255, 190, 0))
			self.UI["ImageBox"].LoadImage("d:/ymir work/ui/game/fishing_timer/get.tga")
