...
class ToolTip(ui.ThinBoard):
	...
	if app.ENABLE_RARITY_SYSTEM:
		RARITY_COLORS = [0xFF5A66EA, 0xFFEA1CF3, 0xFFFFC312]
...
class ItemToolTip(ToolTip):
	...
	if app.ENABLE_ADVANCED_BLOCK_DODGE:
		AFFECT_DICT[item.APPLY_MAGIC] = localeInfo.TOOLTIP_APPLY_MAGIC
	...
	if app.ENABLE_RARITY_SYSTEM and constInfo.ENABLE_RARITY_BASIC_ATTR:
		def __AppendAttackPowerInfo(self, metinSlot = 0):
			minPower = item.GetValue(3)
			maxPower = item.GetValue(4)
			addPower = item.GetValue(5)
			if metinSlot[constInfo.RARITY_ITEM_SOCKET] > 0:
				rarityPct = float(constInfo.RARITY_PERCENT2[metinSlot[constInfo.RARITY_ITEM_SOCKET] - 1])
				if maxPower > minPower:
					self.AppendTextLine(localeInfo.TOOLTIP_ITEM_ATT_POWER % ((minPower+addPower)*rarityPct, (maxPower+addPower)*rarityPct), self.POSITIVE_COLOR)
				else:
					self.AppendTextLine(localeInfo.TOOLTIP_ITEM_ATT_POWER_ONE_ARG % (minPower+addPower*rarityPct), self.POSITIVE_COLOR)
			else:
				if maxPower > minPower:
		...
		def __AppendMagicAttackInfo(self, metinSlot = 0):
			...
			if minMagicAttackPower > 0 or maxMagicAttackPower > 0:
				if metinSlot[constInfo.RARITY_ITEM_SOCKET] > 0:
					...
					if maxMagicAttackPower > minMagicAttackPower:
						self.AppendTextLine(localeInfo.TOOLTIP_ITEM_MAGIC_ATT_POWER % ((minMagicAttackPower+addPower)*rarityPct, (maxMagicAttackPower+addPower)*rarityPct), self.POSITIVE_COLOR)
					else:
						self.AppendTextLine(localeInfo.TOOLTIP_ITEM_MAGIC_ATT_POWER_ONE_ARG % ((minMagicAttackPower+addPower)*rarityPct), self.POSITIVE_COLOR)
				else:
					if maxMagicAttackPower > minMagicAttackPower:
	...
	if app.ENABLE_RARITY_SYSTEM and constInfo.ENABLE_RARITY_BASIC_ATTR:
		def __AppendMagicDefenceInfo(self, metinSlot = 0):
			magicDefencePower = item.GetValue(0)
			if magicDefencePower > 0:
				if metinSlot[constInfo.RARITY_ITEM_SOCKET] > 0:
					rarityPct = float(constInfo.RARITY_PERCENT2[metinSlot[constInfo.RARITY_ITEM_SOCKET] - 1])
					self.AppendTextLine(localeInfo.TOOLTIP_ITEM_MAGIC_DEF_POWER % (magicDefencePower*rarityPct), self.GetChangeTextLineColor(magicDefencePower))
				else:
	...
	if app.ENABLE_RARITY_SYSTEM and constInfo.ENABLE_RARITY_ATTR:
		def __AppendAttributeInformation(self, attrSlot, metinSlot = 0):
			if 0 != attrSlot:
				...
				for i in xrange(player.ATTRIBUTE_SLOT_MAX_NUM):
					...
					if app.ENABLE_RARITY_SYSTEM and constInfo.ENABLE_RARITY_ATTR:
						if metinSlot[constInfo.RARITY_ITEM_SOCKET] > 0:
							rarityPct = float(constInfo.RARITY_PERCENT[metinSlot[constInfo.RARITY_ITEM_SOCKET] - 1])
							value += value * rarityPct
	...
	def __SetNormalItemTitle(self, itemVnum = 0):
		self.SetTitle(str(item.GetItemName()) +" ("+str(itemVnum)+")")

	def __SetSpecialItemTitle(self, itemVnum = 0):
		self.AppendTextLine(str(item.GetItemName())+" ("+str(itemVnum)+ ")", self.SPECIAL_TITLE_COLOR)
	...
	def AddItemData(self, itemVnum, metinSlot, attrSlot = 0, flags = 0, unbindTime = 0, window_type = player.INVENTORY, slotIndex = -1):
		...
		if item.ITEM_TYPE_WEAPON == itemType:
			...
			if item.WEAPON_FAN == itemSubType:
				if app.ENABLE_RARITY_SYSTEM and constInfo.ENABLE_RARITY_BASIC_ATTR:
					...
					self.__AppendAttackPowerInfo(metinSlot)
				else:
			...
			if app.ENABLE_RARITY_SYSTEM and constInfo.ENABLE_RARITY_BASIC_ATTR:
				self.__AppendAffectInformation(metinSlot)
			else:
				self.__AppendAffectInformation()
				
			if app.ENABLE_RARITY_SYSTEM and constInfo.ENABLE_RARITY_BASIC_ATTR:
				self.__AppendAttributeInformation(attrSlot, metinSlot)
			else:
		...
		### Armor ###
			...
			if defGrade > 0:
				self.AppendSpace(5)
				if app.ENABLE_RARITY_SYSTEM and constInfo.ENABLE_RARITY_BASIC_ATTR and metinSlot[constInfo.RARITY_ITEM_SOCKET] > 0:
					rarityPct = float(constInfo.RARITY_PERCENT2[metinSlot[constInfo.RARITY_ITEM_SOCKET] - 1])
					self.AppendTextLine(localeInfo.TOOLTIP_ITEM_DEF_GRADE % ((defGrade+defBonus)*rarityPct), self.GetChangeTextLineColor(defGrade))
				else:
					self.AppendTextLine(localeInfo.TOOLTIP_ITEM_DEF_GRADE % (defGrade+defBonus), self.GetChangeTextLineColor(defGrade))

			if app.ENABLE_RARITY_SYSTEM and constInfo.ENABLE_RARITY_BASIC_ATTR:
				self.__AppendMagicDefenceInfo(metinSlot)
			else:
	...
	if app.ENABLE_RARITY_SYSTEM:
		def AppendTwoColorTextLine(self, text, color, text2, color2, centerAlign = True):
			textLine = ui.TextLine()
			textLine.SetParent(self)
			textLine.SetFontName(self.defFontName)
			textLine.SetPackedFontColor(color)
	...
	if app.ENABLE_RARITY_SYSTEM and constInfo.ENABLE_RARITY_BASIC_ATTR:
		def __AppendAffectInformation(self, metinSlot = 0):
			for i in xrange(item.ITEM_APPLY_MAX_NUM):
				...
				if app.ENABLE_RARITY_SYSTEM and constInfo.ENABLE_RARITY_ATTR:
					if metinSlot != 0:
						if metinSlot[constInfo.RARITY_ITEM_SOCKET] > 0:
							rarityPct = float(constInfo.RARITY_PERCENT[metinSlot[constInfo.RARITY_ITEM_SOCKET] - 1])
							affectValue += affectValue * rarityPct
	...
	if app.ENABLE_RARITY_SYSTEM:
		def __AppendRarityTitle(self, metinSlot):
			if metinSlot:
				...
				if rarity > 0:
					rarity_names = [localeInfo.RARITY_1, localeInfo.RARITY_2, localeInfo.RARITY_3]
					self.AppendTextLine(rarity_names[rarity-1], self.RARITY_COLORS[rarity-1])
		...
		def __AppendRarityInformation(self, metinSlot):
			if metinSlot:
				...
				values = ["5%", "10%", "15%"]
				if rarity > 0:
					self.AppendTextLine(localeInfo.RARITY_INFO_TEXT % values[rarity-1], self.RARITY_COLORS[rarity-1])
