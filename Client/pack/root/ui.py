...
class Gauge(Window):
	...
	def __init__(self):
		Window.__init__(self)
		self.width = 0
		self.diffPercentage = 0.0      # jelenlegi diff csik ertek
		self.currentPercentage = 0.0   # main HP
		self.targetPercentage = 0.0    # a cel HP (diff csiknak)
		self.animStart = 0.0           # animacio kezd? erteke
		self.animTime = 0.0            # eltelt anim id?
		self.animDuration = 0.6        # anim id? (masodperc)
		self.animating = False
		self.lastUpdateTime = 0  # utolso update ideje
	...
	def MakeGauge(self, width, color):
		...
		imgDiffGauge = ExpandedImageBox()
		imgDiffGauge.SetParent(self)
		imgDiffGauge.LoadImage("d:/ymir work/ui/pattern/gauge_orange.tga")  # masik szin!
		imgDiffGauge.Show()
		imgDiffGauge.SetRenderingRect(0.0, 0.0, 0.0, 0.0)
		imgDiffGauge.SetPosition(self.GAUGE_TEMPORARY_PLACE, 0)
		...
		self.imgDiffGauge = imgDiffGauge
	...
	def SetPercentage(self, curValue, maxValue):
		...
		self.imgDiffGauge.SetRenderingRect(0.0, 0.0, 0.0, 0.0)
		
		if percentage < self.currentPercentage:
			self.animStart = self.currentPercentage
	...
	def OnUpdate(self):
		...
		deltaMS = now - self.lastUpdateTime
		...
		# progress 0..1
		progress = self.animTime / self.animDuration
		if progress >= 1.0:
			progress = 1.0
			self.animating = False
		...
		# uj diffPercentage
		self.diffPercentage = self.animStart - (self.animStart - self.targetPercentage) * progress
	
		# Render
		diffSize = -1.0 + float(self.width - self.GAUGE_TEMPORARY_PLACE*2) * self.diffPercentage / self.GAUGE_WIDTH
		self.imgDiffGauge.SetRenderingRect(0.0, 0.0, diffSize, 0.0)
