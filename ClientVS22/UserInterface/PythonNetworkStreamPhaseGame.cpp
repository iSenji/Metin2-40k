	...
	switch (exchange_packet.subheader)
	{
		...
		case EXCHANGE_SUBHEADER_GC_ITEM_ADD:
			if (exchange_packet.is_me)
			{
				...
				CPythonExchange::Instance().SetItemToSelf(iSlotIndex, exchange_packet.arg1, (WORD)exchange_packet.arg3);				for (int i = 0; i < ITEM_SOCKET_SLOT_MAX_NUM; ++i)
			...
			{
				int iSlotIndex = exchange_packet.arg2.cell;
				CPythonExchange::Instance().SetItemToTarget(iSlotIndex, exchange_packet.arg1, (WORD)exchange_packet.arg3);				for (int i = 0; i < ITEM_SOCKET_SLOT_MAX_NUM; ++i)
...
bool CPythonNetworkStream::RecvPartyAdd()
{
	...
	CPythonPlayer::Instance().AppendPartyMember(kPartyAddPacket.pid, kPartyAddPacket.name, kPartyAddPacket.race, kPartyAddPacket.level);
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "AddPartyMember", Py_BuildValue("(isii)", kPartyAddPacket.pid, kPartyAddPacket.name, kPartyAddPacket.race, kPartyAddPacket.level));
#else
