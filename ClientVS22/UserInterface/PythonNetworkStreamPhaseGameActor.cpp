...
void __SetWeaponPower(IAbstractPlayer& rkPlayer, DWORD dwWeaponID)
{
	...
	if (CItemManager::Instance().GetItemDataPointer(dwWeaponID, &pkWeapon))
	{
		if (pkWeapon->GetType()==CItemData::ITEM_TYPE_WEAPON)
		{
			...
			int rarity = CPythonPlayer::Instance().GetItemMetinSocket(TItemPos(INVENTORY, c_Equipment_Weapon), 3);
			const float RARITY_VALUES[] = { 1.0f, 1.05f, 1.1f, 1.15f };
			if (rarity > 0)
			{
				minPower = minPower * RARITY_VALUES[rarity];
				maxPower = maxPower * RARITY_VALUES[rarity];
				minMagicPower = minMagicPower * RARITY_VALUES[rarity];
				maxMagicPower = maxMagicPower * RARITY_VALUES[rarity];
				addPower = addPower * RARITY_VALUES[rarity];
			}
#endif
