...
PyObject * playerGetItemLink(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (pPlayerItem && CItemManager::Instance().GetItemDataPointer(pPlayerItem->vnum, &pItemData))
	{
		...
		len = snprintf(itemlink, sizeof(itemlink), "item:%x:%x:%x:%x:%x:%x:%x:%x:%x",
			pPlayerItem->vnum, pPlayerItem->flags,
			pPlayerItem->alSockets[0], pPlayerItem->alSockets[1], pPlayerItem->alSockets[2], pPlayerItem->alSockets[3], pPlayerItem->alSockets[4], pPlayerItem->alSockets[5], pPlayerItem->alSockets[6]);
#else
...
PyObject* playerGetPartyMemberRace(PyObject* poSelf, PyObject* poArgs)
{
	int iPID;
	if (!PyTuple_GetInteger(poArgs, 0, &iPID))
		return Py_BuildException();
...
#ifdef ENABLE_RARITY_SYSTEM
	PyModule_AddIntConstant(poModule, "METINSTONE_SOCKET_MAX_NUM",				ITEM_METIN_SOCKET_SLOT_MAX_NUM);
#endif
